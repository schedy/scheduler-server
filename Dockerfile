...
RUN dnf -y install ruby rubygems libpq nodejs lz4 unzip  bzip2 procps-ng iproute psmisc mc less aha hostname
...
COPY public/ ./public/
COPY config.ru  ./
RUN mv config/database.yml.devel-compose config/database.yml
